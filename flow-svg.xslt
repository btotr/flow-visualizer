...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/rdf:RDF" >
		...
		<svg version="1.1" onload="new Controller()"   preserveAspectRatio="xMinYMin meet">
			...
			<xsl:call-template name="instruction">
    			...
    			<xsl:with-param name="instruction" select="$firstInstruction" />
	...
	<xsl:template name="instruction">
		...
		<xsl:message>Current instruction: <xsl:value-of select="$instruction/@rdf:about" /> y:<xsl:value-of select="$y" /></xsl:message>
		<g>
		...
		<xsl:for-each select="//rdf:Description[(core:depVariationInstruction/@rdf:resource=$depIRIBase) or (core:depVariationInstruction/@rdf:resource=$currentInstruction)[1]]">
			...
			<xsl:if test="$depIRI or $depIRIBase">
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:message>depcall <xsl:value-of select="$pos" /></xsl:message>
						<xsl:call-template name="instruction">
